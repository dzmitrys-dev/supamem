"""Phase 16 Plan G — README + 4 translations doc-shape tests for v0.3.0a6.
...
def test_readme_mentions_live_three_run_baseline(readme_en: str) -> None:
    """Phase 16 §7 rewrite: README references the live three-run baseline."""
    has_live = any(
        token in readme_en
        for token in ("Live three-run", "live three-run", "live 21,235", "live baseline")
    )
    assert has_live, (
        "README.md must mention the live three-run baseline (Phase 16-E) — "
        "the offline floors were removed in v0.3.0a6"
    )


def test_readme_mentions_5000ms_p95_ceiling(readme_en: str) -> None:
    """Phase 16 D-LAT-01: 500 ms → 5000 ms one-shot ceiling adjustment."""
    assert "5000" in readme_en, (
        "README.md must reference the 5000 ms p95 ceiling (D-LAT-01 one-shot adjustment)"
    )


def test_readme_links_adr_section_7(readme_en: str) -> None:
    """README must deep-link ADR-0002 §7 (live floors)."""
    has_link = (
        "0002-coderag-eval-philosophy.md#7-locked-numerical-floors" in readme_en
        or "ADR-0002](docs/adr/0002-coderag-eval-philosophy.md) §7" in readme_en
    )
    assert has_link, "README.md must deep-link ADR-0002 §7 (live floors)"


def test_readme_links_adr_section_8_mem0(readme_en: str) -> None:
    """README must deep-link the new ADR-0002 §8 mem0 peer comparison."""
    has_link = (
        "0002-coderag-eval-philosophy.md#8-mem0-peer-comparison" in readme_en
        or "ADR-0002 §8" in readme_en
    )
    assert has_link, (
        "README.md must deep-link ADR-0002 §8 (Mem0 peer comparison; new in v0.3.0a6)"
    )


def test_readme_mentions_paired_bootstrap_delta(readme_en: str) -> None:
    """README must surface the paired-bootstrap delta + 95% CI peer comparison."""
    assert "paired-bootstrap" in readme_en, (
        "README.md must mention the `paired-bootstrap` delta + 95% CI for the mem0 peer row"
    )
...
@pytest.mark.parametrize("translation", TRANSLATIONS, ids=lambda p: p.name)
def test_translations_carry_5000ms_ceiling_literal(translation: Path) -> None:
    """v0.3.0a6 Phase 16: the 5000 ms ceiling literal stays canonical English."""
    body = translation.read_text(encoding="utf-8")
    assert "5000" in body, (
        f"{translation.name} missing canonical 5000 ms p95 ceiling literal "
        "(D-LAT-01 one-shot adjustment)"
    )
...
def test_translations_synced_with_sha_matches_last_readme_commit(translation: Path) -> None:
    ...
    (translations still pin the v0.3.0a5 SHA `73f3e33`); Task 2 closes
    the gate by bumping the SHA to point at Task 1's commit.
